...
import com.app.goldenhealth.ui.fragment.CauHoiTraLoiView;
...
public class CauHoiTraLoiPresenterImpl extends BasePresenterImpl<CauHoiTraLoiView> implements CauHoiTraLoiPresenter {

    private static final String TAG = "CauHoiTraLoi";

    public CauHoiTraLoiPresenterImpl(CauHoiTraLoiView view) {
        super(view);
    }

    @Override
    public void traLoi(int cauHoiId, String traLoi){
        Util.getIns().showLoadding(getView().gContext());
        String token = "Bearer " + PrefUtil.getToken(getView().gContext()).getAccessToken();
        String uid = PrefUtil.getDataUser(getView().gContext()).getUID();
        NetworkModule.getService().traLoiCauHoi(token, uid, cauHoiId, new ArrayList<Integer>(), "")
